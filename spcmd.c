...
int main(int argc, char* argv[]) {
    // Check if the program is running in console mode
    HWND consoleWindow = GetConsoleWindow();
    
    // Check if we're running in a console or were double-clicked
    ...
    if (consoleWindow != NULL) {
        ...
        // If the console window doesn't belong to this process, we were double-clicked
        if (consoleProcessId != GetCurrentProcessId()) {
            isDoubleClicked = TRUE;
        }
    } else {
        // No console window at all - likely double-clicked
        isDoubleClicked = TRUE;
    }
    
    // If double-clicked, show info message and exit
    if (isDoubleClicked) {
        ...
        // Show info message and exit
        int result = MessageBoxA(NULL, 
            "SPCMD is a command-line tool.\n\n"
            "Please run it from the Command Prompt or PowerShell with appropriate parameters.\n\n"
            "Example usage:\n"
            "  spcmd screenshot\n"
            "  spcmd infobox \"Hello\" \"Message\"\n\n"
            "Type 'spcmd /help' for more information.", 
            "SPCMD - System Power Command Tool", 
            MB_OK | MB_ICONINFORMATION | MB_TOPMOST);
            
        // Wait for the message box to be closed
        if (result == 0) {
            // If MessageBox failed, try a simple beep
            MessageBeep(MB_OK);
        }
        
        // Add a longer delay to ensure the message box is visible
        Sleep(1000);
        return 0;
    }
